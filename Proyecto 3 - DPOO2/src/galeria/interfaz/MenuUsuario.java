...
/**
 * El objetivo de esta clase es implementar ciertos métodos que son comunes para
 * todos los Usuarios (Empleados y Clientes).
 */
public abstract class MenuUsuario extends MenuBasico
{
	...
	/**
	 * Este método muestra un menú continuo en el cual se pueden seleccionar piezas
	 * para ver sus transacciones repetidamente.
	 * 
	 * @param nombreMensaje
	 * @param nombresPiezas
	 */
	protected void menuMostrarTransaccionesVariasPiezas(String nombreMensaje, List<String> nombresPiezas) {
		...
		int opcionSeleccionada = mostrarMenu("Piezas de " + nombreMensaje, opciones,
	...
	/**
	 * Este método muestra un menú continuo en el cual se pueden seleccionar piezas
	 * para ver su información repetidamente.
	 * 
	 * @param nombreMensaje
	 * @param nombresPiezas
	 */
	protected void menuMostrarInformacionVariasPiezas(String nombreMensaje, List<String> nombresPiezas,
			boolean incluirPropietario, boolean incluirPrecioMinimoSubasta) {
		int numPiezas = nombresPiezas.size();
		String[] opciones = new String[numPiezas + 1];
		Iterator<String> it = nombresPiezas.iterator();
		for (int i = 0; i < numPiezas && it.hasNext(); i++) {
		...
		if (opcionSeleccionada == numPiezas + 1) {
			...
			mostrarInformacionPieza(nombresPiezas.get(opcionSeleccionada - 1), incluirPropietario,
					incluirPrecioMinimoSubasta);
			menuMostrarTransaccionesVariasPiezas(nombreMensaje, nombresPiezas);
		}
	}
	...
	protected void mostrarInformacionPieza(String nombrePieza, boolean incluirPropietario,
		...
		if (pieza.isEnPosesion()) {
			...
			if (pieza.getPrecioUltimaVenta() >= 0) {
				System.out.println("Precio por el cual la pieza se vendió por última vez: "
						+ Long.toString(pieza.getPrecioUltimaVenta()));
			}
			if (incluirPrecioMinimoSubasta)
				System.out.println("Precio mínimo subasta: " + pieza.getPrecioMinimoSubasta());
			System.out.println("Precio inicio subasta: " + pieza.getPrecioInicioSubasta());
			if (pieza.isBloqueada())
				System.out.println("La pieza tiene una oferta en proceso");
			if (pieza.isEnSubasta())
				System.out.println("La pieza tiene una subasta en proceso");
			if (pieza.isEnBodega())
				System.out.println("La pieza se encuentra en la bodega");
			else
				System.out.println("La pieza está siendo exhibida");
		} else {
	...
	protected void mostrarInformacionTransaccion(Transaccion transaccion, boolean incluirNombrePieza) {
		...
		if (incluirNombrePieza)
			System.out.println("Pieza: " + transaccion.getPieza().getTitulo());
		...
		if (tipo == Transaccion.SUBASTA)
			System.out.println("Tipo: Subasta");
		if (tipo == Transaccion.VENTA_DIRECTA)
			System.out.println("Tipo: Venta directa");
	}
